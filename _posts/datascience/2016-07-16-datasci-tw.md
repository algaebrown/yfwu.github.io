...
- 資料決策流程的建立（本次重點）
    - 怎麼樣收集、管理、處理這些資料？
        - 來源
            ...
            - ``DauGA``
                - [Github Repo](https://github.com/junyiacademy/dauGA)
...
- 四個挑戰
    - 規劃面
        - 從定義目標開始：把真實問題限縮到能夠被量化驗證的程度
            - 例如：捐款金額倍增（business goals） => 優化勸募涵的轉換率（marketing gaoals） => 利用 once and done 來提升（conversion goals） => 今年度應該有多少用戶呢？（KPI）（transform goals）。在這四個 goals 中，每個都有多種選項，這些都得**執行過**才知道答案！
